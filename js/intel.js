...
window.toggleIntelCard = function(index, el, event) {
  ...
  const body = el.querySelector('.nexus-body');
  const icon = el.querySelector('.nexus-icon i');
  if (body && icon) {
      body.classList.toggle('hidden');
      icon.setAttribute('data-lucide', body.classList.contains('hidden') ? 'chevron-down' : 'chevron-up');
      if (window.lucide) window.lucide.createIcons();
      db.factors[index].isCollapsed = body.classList.contains('hidden');
...
indexedFactors.forEach(({factor, originalIndex}) => {
  ...
  const firmBadge = factor.linkedFirm ? `<button onclick="routeToFirm('${safeFirm}')" class="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-700 px-2 py-0.5 rounded font-bold shadow-sm border border-slate-200 inline-flex items-center gap-1 transition dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700 uppercase tracking-widest shrink-0"><i data-lucide="building-2" class="w-3 h-3"></i>${factor.linkedFirm}</button>` : '';
  const competencyBadge = (currentWorkspace === "Interview Vault" && factor.competency) ? `<span class="text-[10px] bg-purple-100 text-purple-800 px-2 py-0.5 rounded font-bold shadow-sm border border-purple-200 inline-flex items-center gap-1 shrink-0 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800 uppercase tracking-widest"><i data-lucide="target" class="w-3 h-3"></i>${factor.competency}</span>` : '';
  ...
  card.className = `bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 border-l-4 ${pColor} rounded-md p-4 md:p-6 shadow-sm print:break-inside-avoid print:border-slate-400 print:shadow-none group cursor-pointer transition hover:shadow-md hover:bg-slate-50 dark:hover:bg-slate-900/50`;
  
  card.setAttribute("onclick", `window.toggleIntelCard(${originalIndex}, this, event)`);
    ...
    <div class="flex items-start gap-4 w-full">
      ...
      <div class="flex flex-col min-w-0 w-full">
        ...
        <div class="flex flex-wrap items-center gap-2 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 w-full">
            ...
            ${nexusBadge}${firmBadge}${competencyBadge}${scoreBadge}
        </div>
      </div>
    </div>
...
// Ensure we delete by absolute index
...
if(confirm(`Delete ${selectedFactors.size} selected insight(s)?`)) {
    // Must sort descending so splice doesn't shift remaining indices
...
try {
    ...
    db.factors.push({
        ...
        metric: "AI Parsed",
        summary: safeSummary,
        description: "<p><strong>Source:</strong> AI Parsed from pasted text.</p>",
